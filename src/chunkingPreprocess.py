...
section_pattern = (
    r"^\d+\.\s+([A-Z].*)"  # Matches numbered section headers like "1. Title"
)
...
def load_and_chunk_documents(data_folders, chunk_size=120):
        ...
        for file_path in folder.glob("*.txt"):
            with open(file_path, "r", encoding="utf-8") as file:
                ...
                if content:
                    ...
                    for i in range(
                        1, len(sections), 2
                    ):  # Iterate over section titles and contents
    ...
    if not chunks:
        print(
            "No document chunks found. Please ensure there are valid `.txt` files in the data folders."
        )
    else:
        print(
            f"Loaded {len(chunks)} chunks from documents in the provided data folders."
        )
...
def embed_and_store(chunks, metadata):
    if not chunks:
        raise ValueError(
            "No document chunks to embed. Please check the 'data' directory."
        )
    ...
    with open(mapping_file, "w", encoding="utf-8") as f:
        for meta in metadata:
            f.write(
                f"{meta['Chunk ID']}\t{meta['Document ID']}\t{meta['Section Title']}\t{meta['Subsection']}\t{meta['Folder']}\t{meta['File Name']}\n"
            )

    print(
        f"FAISS index created and saved to {index_file}. Metadata saved to {mapping_file}."
    )
    return index
